...
void conv(hls::stream<int>& out, hls::stream<int>& w, hls::stream<int>& in)
{
  ...
  int j;
  int _lt_var_to;
  for (_lt_var_to = 0; _lt_var_to <= 191; _lt_var_to += 18) {
    int _lt_var_ti;
    for (_lt_var_ti = 0; _lt_var_ti <= 255; _lt_var_ti += 13) {
      int _lt_var_col;
      for (_lt_var_col = 0; _lt_var_col <= 12; _lt_var_col += 7) {
        int _lt_var_row;
        load_w:
        int w_buff[18][13][3][3];
        int hold_w;
        int d;
        int c;
        int b;
...
if(!w.empty()){w>> hold_w; w_buff[a][b][c][d] = hold_w;}
        ...
        for (_lt_var_row = 0; _lt_var_row <= 12; _lt_var_row += 3) {
#pragma HLS DATAFLOW
          load_in:
          int in_buff[13][3][7];
          int hold_in;
...
if(!out.empty()){in>> hold_out; out_buff[a2][b2][c2] = hold_out;}
          ...
          compute:
          for (row = _lt_var_row; row <= (((12 < (_lt_var_row + 3 - 1))?12 : (_lt_var_row + 3 - 1))); row += 1) {
            for (col = _lt_var_col; col <= (((12 < (_lt_var_col + 7 - 1))?12 : (_lt_var_col + 7 - 1))); col += 1) {
              for (i = 0; i < 3; i++) {
                for (j = 0; j < 3; j++) {
                  for (ti = _lt_var_ti; ti <= (((255 < (_lt_var_ti + 13 - 1))?255 : (_lt_var_ti + 13 - 1))); ti += 1) {
#pragma HLS UNROLL
                    for (to = _lt_var_to; to <= (((191 < (_lt_var_to + 18 - 1))?191 : (_lt_var_to + 18 - 1))); to += 1) {
